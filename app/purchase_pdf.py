...
def generate_purchase_pdf(invoice_data: dict, edit_data: dict) -> BytesIO:
    ...
    def draw_totals_section(canvas_obj, data_start_y, totals_start_row, totals_x, totals_box_width, items_height):
        # In the context of a vendor purchase, `total_net_all` increases what we owe them.
        # `previous_balance` from the DB is typically negative for payables.
        # We want to display everything as positive numbers representing "Amount Owed to Vendor".
        ...
        canvas_obj.drawRightString(table_x + table_width - 6, rem_row_y - MIN_ROW_HEIGHT + 5, f'{remaining_balance_display:.3f}')
    ...
    def draw_header(canvas_obj):
        ...
        canvas_obj.drawString(customer_box_x + 6, y_pos, 'VENDOR NAME #')
        ...
        # Header text for Purchase Receipt
        header_text = 'PURCHASE RECEIPT'
        canvas_obj.setFillColor(GRAY_DARK)
        ...
        meta_data = [
            ('Purchase Date:', format_date(invoice_data.get('date'))),
            ('Purchase No:', edit_data.get('invoice_number', '')),
